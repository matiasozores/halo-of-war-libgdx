...
import com.haloofwar.game.components.PlayerDefaultRemoteMovementController;
import com.haloofwar.game.components.PlayerMovementController;
import com.haloofwar.game.components.PlayerRemoteMovementController;
...
public class ComponentPresets {
	...
	public static TransformComponent defaultTransform(int identifier, float x, float y) {
		return new TransformComponent(identifier, x, y, WIDTH, HEIGHT);
	}
	
	public static TransformComponent defaultTransform(int identifier, float x, float y, float width, float height) {
		return new TransformComponent(identifier, x, y, width, height);
	}
	
    public static AnimationComponent defaultAnimation(AnimatedEntityDescriptor descriptor, TextureManager textureManager) {
        return new AnimationComponent(descriptor, textureManager, false, false);
    }
    
    public static AnimationComponent defaultObjectAnimation(AnimatedEntityDescriptor descriptor, TextureManager textureManager, boolean lastState) {
        return new AnimationComponent(descriptor, textureManager, true, lastState);
    }
    ...
    public static MovementComponent remotePlayerMovement(final int IDENTIFIER, EventBus bus) {
		MovementController controller = new PlayerRemoteMovementController(IDENTIFIER, bus);
		return new MovementComponent(controller, DEFAULT_VELOCITY);
	}
    
    public static MovementComponent remoteDefaultPlayerMovement() {
		MovementController controller = new PlayerDefaultRemoteMovementController();
		return new MovementComponent(controller, DEFAULT_VELOCITY);
	}
    ...
    public static EnemyComponent defaultEnemy(final EnemyType type) {
    ...
    public static Component defaultWeaponComponent(Weapon weapon) {
		return weapon.createComponent();
	}
    
    public static Component defaultWeaponBehavior(Weapon weapon) {
    	return weapon.createBehavior();
    }
    ...
    /*public static InventoryComponent inventoryFromData(InventoryData data, final TextureManager TEXTURE) {
        ...
        for (InventoryData.ItemData itemData : data.items) {
            Entity entity = ObjectFactory.createItemByName(itemData.name, TEXTURE);
